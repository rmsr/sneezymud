...
struct classSkillPairT {
    const unsigned short class_num;
    const spellNumT skill_num;
};

static std::unordered_map<spellNumT, std::vector<classSkillPairT>>
classSkillMap = {
...
spellNumT TBeing::getSkillNum(spellNumT skill_num) const
{
    ...
    if (!classSkillMap.count(skill_num))
        return skill_num;

    int best_value = 0;
    for (auto pair : classSkillMap[skill_num]) {
        if (!hasClass(pair.class_num))
            continue;
        int this_value = getSkillValue(pair.skill_num);
